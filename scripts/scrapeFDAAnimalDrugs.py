...
FDAanimalDrugstxt = "../stitcher-inputs/active/FDAanimalDrugs.txt"
oldnadasjson = "../stitcher-inputs/NADAs.json"
newnadasjson = "../stitcher-inputs/temp/NADAs.json"

faradProds = "../stitcher-inputs/faradProds.txt"
faradtxt = "../stitcher-inputs/farad.txt"
...
def resolveNameTripod(name):
    ...
    try:
        ...
        if err.code == 403:  # Check for 403 Forbidden status code
            print("Received 403 Forbidden. Retrying after 1 minute...")
        ...
        for i in range(0, int(r['total'])):
            if '_approvalIDDisplay' in r['content'][i]:
                if unii == '':
                    ...
                    print("conflicting UNIIs returned:")
                    print(name)
                    print(request)
...
def resolveName(name):
    ...
    if unii == "":
        sys.stderr.write("Can not resolve name to a unii: " + str(name) + "\n")
    print(f"{name} => {unii}")
    return unii
...
if __name__ == "__main__":
    ...
    for line in lines[1:-1]:
        ...
        sponsor = ""
        apptype = ""
        if 'proprietaryPreviewBean' in obj:
        ...
        if 'ingredientsPreviewBean' in obj:
        ...
        if 'applicationStatus' in obj:
            status = obj['applicationStatus']
        if 'sponsorPreviewBean' in obj:
            sponsor = obj['sponsorPreviewBean']['sponsorName']
        if 'applicationType' in obj:
    ...
    for app in apps.keys():
        ...
        for item in apps[app]['Ingredients']:
            if not item.upper() in ingreds:
                ingreds[item.upper()] = app
            elif app < ingreds[item.upper()] or (
                    apps[ingreds[item.upper()]]['Status'] != "Approved" and apps[app]['Status'] == 'Approved'):
                ingreds[item.upper()] = app
    ...
    outFile.write(
        "NADA_ID\tUNII\tIngredient\tAppNo\tRx\tStatus\tAppType\tSponsor\tTrade Name\tReference\tRoutes\tIngredients\n")
