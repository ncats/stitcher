...
For data sources with chemical structures, the most computationally demanding step in data ingestion is the generation of molecular hash keys. Hash keys are generated for three different structural levels: \texttt{L5}, \texttt{L4}, and \texttt{L3}, which correspond to stitch keys \texttt{H\_LYCHI\_L5}, \texttt{H\_LYCHI\_L4}, and \texttt{H\_LYCHI\_L3}, respectively. Level \texttt{L5} is the most specific; it represents the molecular structure as-is, i.e., without structure normalization and standardization. A match at this level is preferred over other stitch keys. The next level \texttt{L4} represents the structure after normalization and standardization \citep{lychi}. A match at this level implies that two structures are the same insofar as the valence bond theory is valid. And the last level \texttt{L3} represents chemical structures with incorrect or missing stereochemistry. A match at this level is considered weak and does not constitute equivalence without significant supporting evidence. For each hash key, a suffix \texttt{-M}, \texttt{-S}, or \texttt{-N} is also assigned to denote the molecular component is a \emph{metal}, \emph{salt}, or \emph{neither}, respectively.
