    ...
    with open(file, 'rb') as file:
        ...
        for row in reader:
            ...
            for i in range(0, len(header)):
                ...
                if i < len(row):
                    ...
                    if r != '':
                            ...
                            for vi in range(len(vals)):
                                ...
                                if vals[vi] != '':
                                    ...
                                    if name in conditions:
                                        ...
                                        ht.update(v)
                            # add value to corresponding variable in temp row
                            x[name] = vals
                        elif name == 'Unii':
                            unii = []
                            for v in r.split(';'):
                        ...
                        elif (name == 'TreatmentModality'
                              and r.lower() == 'curative'):
                            x[name] = 'Primary'
                        else:
                            if r.lower() == 'false':
                                x[name] = False
            ...
            if combined_comp_rows == None:
                ...
                # if it exists
                # and does not belong to the same compound as row object...
                if combined_comp_rows['CompoundName'] != current_row_dict['CompoundName']:
                    # dump json to file
                    print json.dumps(combined_comp_rows,
                                     indent=4,
                                     separators=(',', ': ')) + ','
                    ...
                    if c.has_key('id'):
                        ...
                        # create it in the summary object anew
                        else:
                    ...
                    if t.has_key('id'):
                        ...
                        # if it's the first target
                        # then create it in the summary object anew
                        else:
                            combined_comp_rows['Targets'] = [t]
                    # point current row object to summary object
                    current_row_dict = combined_comp_rows

    # in the end either a new row or a summary object
    # point to the same unrecorded object, so it needs to be recorded
