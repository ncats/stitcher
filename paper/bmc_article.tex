...
\begin{abstractbox}
  ...
  \begin{abstract} % abstract
    ...
    the ability to determine entities from different data sources
    with shared partial (and perhaps even inconsistent) identities are 
    equivalent. For many entity types with well-defined nomenclature
    (e.g., gene, protein, cell line, etc.), ER amounts to a simple
    lookup of identifiers. For drug entity type, however, ER is rather
    challenging due to ambiguities in how it is defined and
    ...
    ER framework, \st, for drug data integration. Using \emph{active
      moiety} as the defining concept for drug entity, we 
    develop a set of equivalence relations (i.e., \emph{stitch
      keys}) in \st\ specifically for ER of drug entities. We
    demonstrate the utility of our approach through the development of
    a comprehensive resource of drugs that have either been marketed or
    approved in the United States for human use.
  \end{abstract}
...
frameworks for artificial intelligence, proper entity resolution (ER) is
essential in creating ground-truth data and turning data into
knowledge. The core challenge of ER is in establishing
...
nosology \cite{Hucklenbroich14} is what makes disease ER extremely
challenging.
...
molecular targets \cite{Huang2011,Huang2019}. To the best of our
knowledge, \ix{} is currently the most comprehensive resource of its
kind. In the remainder of this paper, we discuss the development of
\st, an ER framework that we have developed to address the challenges
of drug data integration.

\section*{Overview}
As a motivating example, consider the data shown in
Table~\ref{tab:data-integration-example}, which illustrates a
...
different resolutions \cite{lychi}. Section~\ref{sec:methods-ingest}
...
steps executed in order: \emph{ingestion}, \emph{stitching},
...
shown in Figure~\ref{fig:aspirin}. While the two entities have nothing
in common, in G-SRS 
...
thus far resisted due to other forms of \emph{active moiety}
...
States. Our starting point is the public G-SRS data source from the
FDA \cite{GSRSData}. This data source is well-curated and contains
over 100K substances across six different classes: chemical,
structurally diverse, protein, mixture, polymer, and nucleic acid. As
...
multigraph. All figures and examples used throughout this paper have
been generated directly from this database. 
